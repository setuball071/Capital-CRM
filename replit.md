...
*   **Material de Apoio (Support Materials)**: A resource library organized by categories (Tabelas, Criativos, Processos, Tutoriais). "Tabelas" category replaced by an intelligent commission table simulator that ranks tables by profitability. "Criativos" category replaced by a creative gallery module supporting personalizable (canvas-based customization) and direct download creatives.
...
*   **WhatsApp Webhook Integration**: Receives label events (applied/removed) from an external WhatsApp platform via `POST /api/webhooks/whatsapp-labels` (public, secret-validated). Matches clients by CPF in `clientes_pessoa`, stores labels in `whatsapp_labels` text array. All events audited in `webhook_logs` table. Labels displayed as badges in vendas-atendimento and vendas-consulta. "Meu WhatsApp" shortcut button in header opens external platform URL with `?crm_user_id={id}`. Integration endpoint `GET /api/integration/users` (secret-validated) exposes active user IDs for WhatsApp platform mapping. CRM IDs reference table visible to master on users page. Env vars: `WHATSAPP_WEBHOOK_SECRET`, `VITE_WHATSAPP_PLATFORM_URL`, `CRM_INTEGRATION_SECRET`.
