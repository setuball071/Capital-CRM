...
*   **Simulador de Portabilidade**: Full portability simulator at `/simulador-portabilidade` (replaced former "Simulador de Amortização"). Two-panel layout: Left panel calculates "Contrato Novo" (from margem/coeficiente/prazo/IOF) with calc mode toggle (Por Parcela / Por Valor do Contrato), Right panel calculates "Contrato Final" (after portability with new taxa). Features amortization strategy cards for multiple prazos, detailed month-by-month chronogram table, and PDF export with client data dialog (Nome, CPF masked as `000.***.***-00`, Convênio) and logged-in user's name/email as corretor data. PDF validity date is always tomorrow (current date + 24h). Uses custom branded CSS (inline scoped styles) per user specification. Client-side only, no backend API needed. PDF export includes tenant logo (pre-converted to base64 via canvas), dark header with logo + date, gradient corretor bar, client info bar, styled summary cards, and professional table with dark header row.
*   **Material de Apoio (Support Materials)**: Resource library at `/material-apoio` organized by categories (Tabelas, Criativos, Processos, Tutoriais). Header shortcuts provide quick navigation to each category — context-aware: on sales screens (`/vendas/campanhas`, `/vendas/atendimento`, `/vendas/consulta`, `/vendas/pipeline`), Tabelas/Criativos/Tutoriais open a centered modal overlay; Processos always opens in a new browser tab; on other screens, normal navigation. Grid of cards with type icons (PDF, Video, Image, Link), category badges, descriptions, and "Abrir" button opening URLs in new tab. Master/coordenacao can add/remove materials via dialog. All authenticated users can view. Data stored in `materials` table with tenant isolation. API: `GET/POST/DELETE /api/materials`. **Tabelas category** replaced by an intelligent commission table simulator: `commission_tables` table stores convênio, banco, tipoProduto, prazo, coeficiente (numeric 10,6), and pontos (numeric 10,4) per tenant. Users select filters and input parcela or valor do contrato, and the system ranks tables by profitability (pontos DESC, top 10). Master/coordenacao can CRUD commission tables. API: `GET/POST/PUT/DELETE /api/commission-tables`, `GET /api/commission-tables/simulate`. Component: `TabelasSimulator` renders in both the page and the modal when category is "tabelas".
*   **CRM de Vendas (Sales CRM)**: Campaign-based lead management system with lead distribution, transfer, status tracking, and integration with the client database. Features tag-based client organization with CSV export (master/coordenacao only), "Lista Manual" (formerly "Atendimento") with "Sair da Lista" and "Próximo" confirmation dialog (Pular / Recusar), and RECUSADO lead marker for tracking refused clients in gestão de pipeline. **Consulta Individual** (`/vendas/consulta`) supports tagging any client via auto-created "Consulta Avulsa" campaign (origem `__consulta_avulsa__`) — if no existing lead matches the CPF, one is created automatically so TagManager always has a leadId. **Exportar Leads** (`GET /api/vendas/campanhas/:id/exportar-leads`) exports campaign leads as CSV with client data joins (pessoa, vinculo, folha). **Importar Higienizados** (`POST /api/vendas/importar-higienizados` + `/vendas/importar-higienizados` page) imports enriched CSV files to create new campaigns with enriched fields (matricula, taxaReal, valorParcela, bancoOferta, dadosHigienizados). Auto-detects CSV columns case-insensitively. Updates clientes_vinculo/folha_mes if enriched data present. sales_leads table has enrichment fields: matricula, taxa_real, valor_parcela, banco_oferta, dados_higienizados (jsonb), higienizado_em.
...
*   **Base de Clientes (Client CRM)**: Comprehensive client database management, supporting Excel/CSV imports, advanced filtering, purchase list requests with package-based pricing, and sophisticated deduplication logic. Includes a high-performance streaming export system for large datasets. **Compra Lista** features a "Base de Referência" selector that scopes folha and D8 contract filters to a specific competência (YYYYMM), ensuring users always query the desired import base. The selector derives `base_tag` suffixes (`fo` for folha, `d8` for contracts) from the selected YYYYMM reference. API endpoint `/api/clientes/filtros/bases` lists available bases. **Bank Exclusion Filter**: `bancos_excluir` field in `filtrosPedidoListaSchema` allows filtering clients who do NOT have contracts at specified banks using a `NOT EXISTS` subquery (respects `base_ref` scoping). UI toggle button switches between "Com estes bancos" (inclusion, default) and "Sem estes bancos" (exclusion) modes.
