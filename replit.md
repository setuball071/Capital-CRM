...
*   **Simulador de Portabilidade**: Full portability simulator at `/simulador-portabilidade` (replaced former "Simulador de Amortização"). Two-panel layout: Left panel calculates "Contrato Novo" (from margem/coeficiente/prazo/IOF), Right panel calculates "Contrato Final" (after portability with new taxa). Features amortization strategy cards for multiple prazos, detailed month-by-month chronogram table, and PDF export with logged-in user's name/email as corretor data. Uses custom branded CSS (inline scoped styles) per user specification. Client-side only, no backend API needed. PDF export includes tenant logo (pre-converted to base64 via canvas), dark header with logo + date, gradient corretor bar, styled summary cards, and professional table with dark header row.
