...
*   **Material de Apoio (Support Materials)**: Resource library at `/material-apoio` organized by categories (Tabelas, Criativos, Processos, Tutoriais). Header shortcuts provide quick navigation to each category. Grid of cards with type icons (PDF, Video, Image, Link), category badges, descriptions, and "Abrir" button opening URLs in new tab. Master/coordenacao can add/remove materials via dialog. All authenticated users can view. Data stored in `materials` table with tenant isolation. API: `GET/POST/DELETE /api/materials`.
