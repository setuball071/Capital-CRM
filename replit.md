...
*   **Usuário Demo (Modo Apresentação)**: Users with `is_demo=true` always show 70% general goal and 90% card goal on their vendor dashboard, regardless of real production. Master can toggle via `isDemo` checkbox in user edit form (Usuários page). The `GET /api/dashboard-vendedor` API injects synthetic data when `user.isDemo && metaMensal > 0`. DB column: `users.is_demo`. Migration: `migrations/add-is-demo-user.sql`.
...
*   **Central de Atualizações**: System update notification manager. Master-only admin page at `/admin/atualizacoes` (sidebar: Administração > Central de Atualizações). Features: create/edit/delete updates with AI-generated content (GPT-4.1-mini), role-based targeting (todos/master/coordenacao/vendedor/parceiro), read tracking with percentage display. A blocking popup (`UpdatesPopup.tsx`, mounted in `App.tsx`) shows unread updates to targeted users in order — cannot be dismissed until "Entendi" is clicked. Tables: `system_updates`, `system_update_reads`. Routes: `GET/POST /api/system-updates`, `PUT/DELETE /api/system-updates/:id`, `POST /api/system-updates/generate`, `GET /api/system-updates/pending`, `POST /api/system-updates/:id/read`, `GET /api/system-updates/:id/reads`.
*   **Informações Complementares por CPF**: CSV import system for CPF+observation text, accessible to master/coordenacao/financeiro roles. Admin import page at `/admin/importar-observacoes` (sidebar: Administração > Observações por CPF). Info icon button appears next to "Dados do Cliente" CardTitle in `vendas-consulta.tsx` and `consulta-cliente.tsx` when an observation exists for the client's CPF, opening a Dialog with the observation text and import date. Table: `client_observations` (unique per tenant+CPF, upsert on re-import). Routes: `GET /api/client-observations/:cpf`, `POST /api/client-observations/import`. Migration: `migrations/client-observations.sql`.
*   **Carteira de Clientes (Portfolio Lock System)**: Client portfolio lock system where a vendedor who closes a contract locks that client for a configurable period per product type, preventing other vendors from approaching them. Managed in `server/portfolio.ts` with `addToPortfolio()`, `checkPortfolioBlock()`, `updateExpiredPortfolios()`. Tables: `portfolio_rules` (configurable duration per product type), `client_portfolio` (active locks with expiry), `portfolio_transfers` (history). Product types: CARTAO, CONSIGNADO, NOVO, PORTABILIDADE, REFINANCIAMENTO. Hooks: auto-adds clients when contracts reach PAGO status (`server/contracts.ts`) and when importing production data (`POST /api/gestao-comercial/importar/confirmar`). Bulk CPF pre-filter in `POST /api/vendas/importar-higienizados` removes blocked CPFs before campaign insertion. Admin page at `/admin/carteira-regras` (sidebar: Administração > Regras de Carteira, masterOnly). "Minha Carteira" tab in `/vendas/pipeline` shows vendor's portfolio cards. CPF block check in `vendas-consulta.tsx` buscarMutation prevents vendedores from looking up clients in another vendor's active portfolio. 24h setInterval in `server/index.ts` runs `updateExpiredPortfolios()`. DB migration: `migrations/carteira-clientes.sql`. Routes: `GET /api/portfolio`, `GET /api/portfolio/check/:cpf`, `POST /api/portfolio/transfer`, `GET /api/portfolio/transfers/:portfolioId`, `GET/PUT /api/portfolio/rules/:productType`.
