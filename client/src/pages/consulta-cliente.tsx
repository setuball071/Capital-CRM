...
/**
 * CENARIO DA REVERSAO DA MP
 * -------------------------------------------------------------------------
 * Hoje (MP, desde mai/2026): teto UNICO de 40% da base de calculo, e o cartao
 * de credito e o de beneficio consomem esse mesmo balde.
 * Se a MP caducar, volta a Lei 14.509/2022: tres baldes INDEPENDENTES --
 * 35% emprestimo + 5% cartao de credito + 5% cartao beneficio. O cartao sai do
 * balde do emprestimo e ganha teto proprio.
 *
 * Por isso a conta se resume a:
 *     margem nova = margem de hoje - 5% da base + parcelas de cartao em folha
 * e quem usa MUITO cartao pode GANHAR margem.
 *
 * O teto de 70% NAO e tocado pela MP e continua rebaixando qualquer sobra: ha
 * cliente que "ganha" no balde do emprestimo e nao consegue usar nada porque a
 * compulsoria esta estourada. Por isso o vendavel e sempre min(balde, saldo 70%).
 *
 * Conferido em 31/08/2026 contra o extrato do CPF 945.979.953-15:
 * hoje -R$ 44,35, pos-MP R$ 190,39 -- exatamente a margem real dele em abril,
 * quando a regra ainda era 35+5+5.
 */
const PCT_LEI = 0.35;      // Lei 14.509 - so emprestimo
const PCT_CARTAO = 0.05;   // cada cartao, com teto proprio

function utilizadoFaixa(
  bruta: number | null | undefined,
  utilizada: number | null | undefined,
  saldo: number | null | undefined,
): number {
  if (utilizada != null) return Number(utilizada);
  const b = bruta != null ? Number(bruta) : null;
  const sd = safeSaldo(saldo, bruta, utilizada);
  if (b != null && sd != null) return b - sd;
  return 0;
}

function margemPosMP(f: FolhaAtual | null | undefined) {
  if (!f) return null;
  const bruta70 = f.margem_bruta_70 != null ? Number(f.margem_bruta_70) : null;
  const bruta35 = f.margem_bruta_35 != null ? Number(f.margem_bruta_35) : null;
  if (bruta70 == null || bruta70 <= 0 || bruta35 == null) return null;

  const saldo35 = safeSaldo(f.margem_saldo_35, f.margem_bruta_35, f.margem_utilizada_35);
  const saldo70 = safeSaldo(f.margem_saldo_70, f.margem_bruta_70, f.margem_utilizada_70);
  if (saldo35 == null) return null;

  const base = bruta70 / 0.70;
  const utilCC = utilizadoFaixa(f.margem_bruta_5, f.margem_utilizada_5, f.margem_saldo_5);
  const utilCB = utilizadoFaixa(
    f.margem_beneficio_bruta_5, f.margem_beneficio_utilizada_5, f.margem_beneficio_saldo_5);
  const utilEmprestimo = (bruta35 - saldo35) - utilCC - utilCB;

  const novaEmprestimo = PCT_LEI * base - utilEmprestimo;
  const hoje = saldo70 != null ? Math.min(saldo35, saldo70) : saldo35;
  const depois = saldo70 != null ? Math.min(novaEmprestimo, saldo70) : novaEmprestimo;
  ...
  return {
    ...
    ganho: depois - hoje,
    novaEmprestimo,
    novaCC: PCT_CARTAO * base - utilCC,
    novaCB: PCT_CARTAO * base - utilCB,
    utilCC,
    utilCB,
    saldo70,
    limitadoPor70: saldo70 != null && novaEmprestimo > saldo70,
    destrava: hoje < 0 && depois >= 0,
  };
}
                ...
                <CardContent>
                  {folhaAtual ? (
                    <div className="space-y-6">
                          ...
                          <CardContent className="p-4">
                            ...
                            <div className="space-y-1 text-sm">
                              ...
                              <div className="flex justify-between font-medium">
                                <span>Saldo:</span>
                                <span className={(safeSaldo(folhaAtual.margem_saldo_70, folhaAtual.margem_bruta_70, folhaAtual.margem_utilizada_70) ?? 0) >= 0 ? "text-green-600" : "text-red-600"}>
                                  {formatCurrency(safeSaldo(folhaAtual.margem_saldo_70, folhaAtual.margem_bruta_70, folhaAtual.margem_utilizada_70))}
                                </span>
                              </div>
                            </div>
                          </CardContent>
                        ...
                        <Card className="bg-muted/50" data-testid="card-margem-35">
                          <CardContent className="p-4">
                            ...
                            <div className="space-y-1 text-sm">
                              ...
                              <div className="flex justify-between font-medium">
                                <span>Saldo:</span>
                                <span className={(safeSaldo(folhaAtual.margem_saldo_35, folhaAtual.margem_bruta_35, folhaAtual.margem_utilizada_35) ?? 0) >= 0 ? "text-green-600" : "text-red-600"}>
                                  {formatCurrency(safeSaldo(folhaAtual.margem_saldo_35, folhaAtual.margem_bruta_35, folhaAtual.margem_utilizada_35))}
                                </span>
                              </div>
                            </div>
                          </CardContent>
                      ...
                      {/* Cenario da reversao da MP -- projecao, nao substitui os numeros acima */}
                      {(() => {
                        const mp = margemPosMP(folhaAtual);
                        if (!mp) return null;
                        const sobe = mp.ganho > 0.01;
                        const desce = mp.ganho < -0.01;
                        return (
                          <Card
                            className={mp.destrava
                            ...
                            data-testid="card-cenario-pos-mp"
                          >
                            <CardContent className="p-4">
                              <div className="flex items-start justify-between gap-3 mb-3">
                                <div>
                                  <p className="text-sm font-semibold">
                                    Cenario se a MP caducar &mdash; 35% + 5% + 5% separados
                                  </p>
                                  <p className="text-xs text-muted-foreground mt-0.5">
                                    Projecao. Os numeros acima continuam sendo a folha de hoje.
                                  </p>
                                </div>
                                {mp.destrava && (
                                  <span className="text-xs font-semibold px-2 py-1 rounded bg-green-600 text-white whitespace-nowrap">
                                    DESTRAVA
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                                <div className="bg-background/70 rounded-lg p-3">
                                  <p className="text-xs text-muted-foreground mb-1">Vendavel hoje</p>
                                  <p className={mp.hoje >= 0 ? "text-lg font-bold text-green-600" : "text-lg font-bold text-red-600"}>
                                    {formatCurrency(mp.hoje)}
                                  </p>
                                </div>
                                <div className="bg-background/70 rounded-lg p-3">
                                  <p className="text-xs text-muted-foreground mb-1">Vendavel depois</p>
                                  <p className={mp.depois >= 0 ? "text-lg font-bold text-green-600" : "text-lg font-bold text-red-600"}>
                                    {formatCurrency(mp.depois)}
                                  </p>
                                </div>
                                <div className="bg-background/70 rounded-lg p-3">
                                  <p className="text-xs text-muted-foreground mb-1">Diferenca</p>
                                  <p className={sobe ? "text-lg font-bold text-green-600" : desce ? "text-lg font-bold text-red-600" : "text-lg font-bold"}>
                                    {sobe ? "+" : ""}{formatCurrency(mp.ganho)}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                                <div className="flex justify-between sm:block">
                                  <span className="text-muted-foreground text-xs">Emprestimo 35%</span>
                                  <span className="font-medium sm:block">{formatCurrency(mp.novaEmprestimo)}</span>
                                </div>
                                <div className="flex justify-between sm:block">
                                  <span className="text-muted-foreground text-xs">Cartao credito 5%</span>
                                  <span className="font-medium sm:block">{formatCurrency(mp.novaCC)}</span>
                                </div>
                                <div className="flex justify-between sm:block">
                                  <span className="text-muted-foreground text-xs">Cartao beneficio 5%</span>
                                  <span className="font-medium sm:block">{formatCurrency(mp.novaCB)}</span>
                                </div>
                              </div>

                              {mp.limitadoPor70 && (
                                <p className="text-xs text-amber-700 dark:text-amber-400 mt-3 p-2 rounded bg-amber-50 dark:bg-amber-950 border-l-2 border-amber-500">
                                  &#x26A0; O emprestimo abriria {formatCurrency(mp.novaEmprestimo)}, mas o teto
                                  de 70% so permite {formatCurrency(mp.saldo70)}. A margem existe no papel e
                                  nao pode ser usada &mdash; para vender, tem que liberar espaco na compulsoria.
                                </p>
                              )}

                              <p className="text-[11px] text-muted-foreground mt-3">
                                Base de calculo {formatCurrency(mp.base)} &middot; perde 5% da base
                                ({formatCurrency(PCT_CARTAO * mp.base)}) e recupera o cartao que sai do
                                balde ({formatCurrency(mp.utilCC + mp.utilCB)}).
                              </p>
                            </CardContent>
                          </Card>
                        );
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-3 bg-muted/50 rounded-lg">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex justify-between font-medium">
                            <span>Saldo:</span>
                            <span className={(safeSaldo(selectedHistoricoCompetencia.margem_saldo_70, selectedHistoricoCompetencia.margem_bruta_70, selectedHistoricoCompetencia.margem_utilizada_70) ?? 0) >= 0 ? "text-green-600" : "text-red-600"}>
                              {formatCurrency(safeSaldo(selectedHistoricoCompetencia.margem_saldo_70, selectedHistoricoCompetencia.margem_bruta_70, selectedHistoricoCompetencia.margem_utilizada_70))}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-muted/50 rounded-lg">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex justify-between font-medium">
                            <span>Saldo:</span>
                            <span className={(safeSaldo(selectedHistoricoCompetencia.margem_saldo_35, selectedHistoricoCompetencia.margem_bruta_35, selectedHistoricoCompetencia.margem_utilizada_35) ?? 0) >= 0 ? "text-green-600" : "text-red-600"}>
                              {formatCurrency(safeSaldo(selectedHistoricoCompetencia.margem_saldo_35, selectedHistoricoCompetencia.margem_bruta_35, selectedHistoricoCompetencia.margem_utilizada_35))}
                            </span>
                          </div>
                        </div>
                      </div>
