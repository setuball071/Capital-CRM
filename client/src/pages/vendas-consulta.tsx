 ...
 *
 * Conferido em 31/08/2026 contra extratos reais:
 *   LUIS THALYS  945.979.953-15  hoje -44,35 -> depois 190,39 (destrava)
 *   JOSE AMARINO 431.462.104-44  hoje   0,12 -> depois   0,12 (travado no 70%)
 * O 190,39 do Luis e exatamente a margem real dele em abril/2026, quando a
 * regra ainda era 35+5+5.
 */
const PCT_LEI = 0.35;      // Lei 14.509 - so emprestimo
const PCT_CARTAO = 0.05;   // cada cartao, com teto proprio

interface FaixasFolha {
...
/** Saldo nunca pode passar de (bruta - utilizada) -- corrige dado importado torto. */
function saldoSeguro(
  saldo: number | null | undefined,
  bruta: number | null | undefined,
  utilizada: number | null | undefined,
): number | null {
  if (bruta != null && utilizada != null) {
    const calculado = Number(bruta) - Number(utilizada);
    return saldo == null ? calculado : Math.min(Number(saldo), calculado);
  }
  return saldo != null ? Number(saldo) : null;
}
...
function utilizadoFaixa(
  ...
  const sd = saldoSeguro(saldo, bruta, utilizada);
  return b != null && sd != null ? b - sd : 0;
}

function margemPosMP(f: FaixasFolha | null | undefined) {
  if (!f) return null;
  ...
  const saldo35 = saldoSeguro(f.margem_saldo_35, f.margem_bruta_35, f.margem_utilizada_35);
  const saldo70 = saldoSeguro(f.margem_saldo_70, f.margem_bruta_70, f.margem_utilizada_70);
                  ...
                  {consultaData.folhaAtual ? (
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        {(() => {
                          const mp = margemPosMP(consultaData.folhaAtual as FaixasFolha);
                          if (!mp) return null;
                          ...
                          return (
                            <Card
                              className={mp.destrava
                                ? "mt-4 border-green-300 dark:border-green-800 bg-green-50/50 dark:bg-green-950/30"
                                : "mt-4 border-blue-200 dark:border-blue-900 bg-blue-50/40 dark:bg-blue-950/20"}
                              ...
                              <CardContent className="p-4">
                                <div className="flex items-start justify-between gap-3 mb-3">
                                  <div>
                                    <p className="text-sm font-semibold">
                                      Cenário se a MP caducar &mdash; 35% + 5% + 5% separados
                                    </p>
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                      Projeção. Os números acima continuam sendo a folha de hoje.
                                    </p>
                                  </div>
                                  {mp.destrava && (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                                  <div className="bg-background/70 rounded-lg p-3">
                                    <p className="text-xs text-muted-foreground mb-1">Vendável hoje</p>
                                    <p className={mp.hoje >= 0 ? "text-lg font-bold text-green-600" : "text-lg font-bold text-red-600"}>
                                      {formatCurrency(mp.hoje)}
                                    </p>
                                  </div>
                                  <div className="bg-background/70 rounded-lg p-3">
                                    <p className="text-xs text-muted-foreground mb-1">Vendável depois</p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                                  <div className="flex justify-between sm:block">
                                    <span className="text-muted-foreground text-xs">Empréstimo 35%</span>
                                    <span className="font-medium sm:block">{formatCurrency(mp.novaEmprestimo)}</span>
                                  </div>
                                  <div className="flex justify-between sm:block">
                                    <span className="text-muted-foreground text-xs">Cartão crédito 5%</span>
                                    <span className="font-medium sm:block">{formatCurrency(mp.novaCC)}</span>
                                  </div>
                                  <div className="flex justify-between sm:block">
                                    <span className="text-muted-foreground text-xs">Cartão benefício 5%</span>
                                ...
                                {mp.limitadoPor70 && (
                                  <p className="text-xs text-amber-700 dark:text-amber-400 mt-3 p-2 rounded bg-amber-50 dark:bg-amber-950 border-l-2 border-amber-500">
                                    &#x26A0; O empréstimo abriria {formatCurrency(mp.novaEmprestimo)}, mas o teto
                                    de 70% só permite {formatCurrency(mp.saldo70)}. A margem existe no papel e
                                    não pode ser usada &mdash; para vender, tem que liberar espaço na compulsória.
                                  </p>
                                )}

                                <p className="text-[11px] text-muted-foreground mt-3">
                                  Base de cálculo {formatCurrency(mp.base)} &middot; perde 5% da base
                                  ({formatCurrency(PCT_CARTAO * mp.base)}) e recupera o cartão que sai do
