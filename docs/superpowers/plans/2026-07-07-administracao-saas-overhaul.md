...
> - ✅ **Fase 4** concluída (commits `a62a1f7` backend + `1b718e0` frontend) — assinaturas religadas ao Asaas: ativar cria recorrência (cancela antiga antes), suspender/cancelar cortam no gateway, webhook renova período + payment_history + notifica; telas com preços canônicos, badge Asaas, adicionais e histórico. Reuso honesto: `gateway_subscription_id`/`current_period_end` em vez de colunas novas; preços de `PLAN_PRICES` (tabela `planos` segue vazia até o catálogo fechar).
> - ✅ **Fase 5** concluída (commits `8359274` backend + `550302a` frontend) — painel SaaS de Ambientes: status+soft/hard delete (dump JSON + confirmKey), wizard 4 passos com provisionamento completo (`server/provisioning.ts`: template do interno, admin do cliente, módulos do plano, assinatura c/ Asaas, domínio, e-mail), `server/railway.ts` (domínio próprio via API), ficha com métricas/módulos toggles, `ultimo_acesso` no login. **Runtime pendente do Fábio:** `WILDCARD_BASE_DOMAIN` + `RAILWAY_API_TOKEN` no Railway (sem eles o wizard funciona, mas sem subdomínio automático/domínio próprio). Fluxo self-serve público (signup) NÃO existe e não foi criado — provisionamento é disparado pelo dono no wizard (o plano citava onboarding pós-pagamento; sem página pública de venda, o gatilho é o wizard).
>
> **PLANO 100% EXECUTADO (8/8 fases).** Falta só runtime/conferência: religar webhook Asaas, definir WILDCARD_BASE_DOMAIN + RAILWAY_API_TOKEN, fechar catálogo módulos×plano e testar no app.
